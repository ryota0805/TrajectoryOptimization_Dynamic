...
def vis_env():
    ...
    d_obs_data = dynamic_obstacle.DynamicObstacle(p.n, p.global_N)
    d_obs_data_list = d_obs_data.dynamic_obs_data
    
    obs_circle = obs_circle + d_obs_data_list
...
def compare_path(trajectory_vector1, trajectory_vector2):
    ...
    #startとgoalを配置
    ax.scatter([x2[0]], [y2[0]], marker='v', color='green', label='start')
